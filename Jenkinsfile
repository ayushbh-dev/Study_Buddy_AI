pipeline {
     agent any
     environment {
        DOCKER_HUB_REPO = "ayushbh01/studybuddy"
    ...
    stages {
        stage('Checkout Github') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'github-token', url: 'https://github.com/ayushbh-dev/Study_Buddy_AI.git']])
            }
        }        
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: ayushbh01/studybuddy:.*|image: ayushbh01/studybuddy:${IMAGE_TAG}|' manifests/deployment.yaml
                    """
                }
            }
        }
